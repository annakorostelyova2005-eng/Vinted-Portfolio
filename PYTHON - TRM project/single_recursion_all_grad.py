...
class RecurrenceEngine(nn.Module):
    ...
    Recursion-only version (all cycles trainable):
    ...
    # ------------------------------------------------------------------
    # Forward: one TRM recursion step (all cycles with gradients)
    # ------------------------------------------------------------------
    ...
    def forward(
        ...
        NOTE: All H_cycles × L_cycles are trainable; we do not use torch.no_grad()
              inside this recursion. Detaching between *supervision* steps (for ACT)
              is still handled by the higher-level TRM wrapper.
        """
        solution = state.solution
        reasoning = state.reasoning

        # Run H_cycles outer loops, each with L_cycles reasoning updates and 1 solution update
